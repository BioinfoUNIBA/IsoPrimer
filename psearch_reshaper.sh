...
for e in ${TEST[*]}; do
	START=${IDX[(($e-1))]}
	STOP=$((${IDX[$e]}-1))
	MATCH=$(sed -n "${START},${STOP}p" $INPUT | grep -oP 'ENS[^T]*?G.*?\.' | tr -d '\.')
	TMATCH=($(sed -n "$(($START+1)),${STOP}p" $INPUT | grep -oP 'ENS[^G]*?T.*' | cut -d. -f1))
	GENE=$(sed -n "${START},${STOP}p" $INPUT | grep Primer | cut -d' ' -f3)
	OUT=($(grep "length: [0-9]* bp" <(sed -n "${START},${STOP}p" $INPUT) | cut -d$' ' -f3))
	LONG=$(for k in ${!OUT[*]}; do j=${OUT[$k]} && (( $j > $MAXLENGTH )) && echo "${TMATCH[$k]}_$j"; done)
	ALL=$(for k in ${!OUT[*]}; do j=${OUT[$k]} && echo "${TMATCH[$k]}_$j"; done)
